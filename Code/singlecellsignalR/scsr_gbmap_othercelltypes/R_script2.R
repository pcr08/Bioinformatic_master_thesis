...
names2 <- row.names(dataset11) #other cell types
...
# Select cells (cell ids) with as maximum 6000 unique genes (upper limit histogram). Upper limit is to make sure you don’t have any outliers that are dying cells or specific to just one or few samples. The cells or spots that are not uniformly distributed across all samples.
...
x11<-rep(c(2),each=1418) #mural cell 1418
...
signal <- cell_signaling(data = data, c.names = c("glioblastoma","mural cells"), genes = rownames(data), cluster = clust)
...
inter.net <- inter_network(data = data, c.names = c("glioblastoma","mural cells"), signal = signal, genes = rownames(data), cluster = clust)
