...
dataset2 <- data_newexp@meta.data[data_newexp@meta.data$cell_type == "radial glial cell",] # radial glial cell types
...
#combine datasets
se_combined.subset <- merge(se_m.subset, y = se_neo.subset, add.cell.ids = c("radial_cells", "neoplastic"))
...
mmatrix <- GetAssayData(object = se_combined.subset, assay= "SCT", slot = "data") #get the normalized UMI counts for both GBM and radial glial cells


library(SCOPfunctions)
data <- utils_big_as.matrix(mmatrix)
nc <- ncol(data[,1:2807]) + 2808
new_mat <- matrix(0, ncol=nc,nrow=nrow(data[,1:2807]))
new_mat[1:nrow(data), 1:ncol(data[,1:2807])] <- data[,1:2807] #new matrix with the values from old one (with radial glial cells) and the rest 0 to add noise

number_fake_cells = 2807
gamma = 0.9
r1 <- 2809
c1 <- 5615
...
# setwd('C:/Users/PAULA CAMARGO ROMERA/OneDrive - Uppsala universitet/Master/Uppsala University/Courses/Year 2/Thesis/processing/cellphonedb_results/gbmap_celltypes/radial_cells')
# p <- 0
# for (j in 2808:2857){
#   p <- p + 1
#   x1<-rep(c("radial_cells"),each=2807) #radial glial cells
#   x2<-rep(c("neoplastic"),each=2808) #glioblastoma 1 patients lab data + noise
# 
#   l1 <- str_split(se_combined.subset@assays$SCT@data@Dimnames[[2]], '"')
#   dd1 <- c(l1[1:2807])
#   dd2 <- c(l1[j])
#   v1 <- c(1:2807) #random names for the noise
#   dd2 <- c(dd2, v1)
# 
