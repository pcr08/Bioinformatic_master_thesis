...
dataset2 <- data_newexp@meta.data[data_newexp@meta.data$cell_type == "astrocyte",] # astrocyte cell types
...
#combine datasets
se_combined.subset <- merge(se_m.subset, y = se_neo.subset, add.cell.ids = c("astrocytes", "neoplastic"))
...
mmatrix <- GetAssayData(object = se_combined.subset, assay= "SCT", slot = "data") #get the normalized UMI counts for both GBM and astrocytes


library(SCOPfunctions)
data <- utils_big_as.matrix(mmatrix)

nc <- ncol(data[,1:173]) + 174
new_mat <- matrix(0, ncol=nc,nrow=nrow(data[,1:173]))
new_mat[1:nrow(data), 1:ncol(data[,1:173])] <- data[,1:173] #new matrix with the values from old one (with astrocytes) and the rest 0 to add noise

number_fake_cells = 173
gamma = 0.9
r1 <- 175
c1 <- 347
...
# setwd('C:/Users/PAULA CAMARGO ROMERA/OneDrive - Uppsala universitet/Master/Uppsala University/Courses/Year 2/Thesis/processing/cellphonedb_results/gbmap_celltypes/astro_cells')
# p <- 0
# for (j in 174:223){
#   p <- p + 1
#   x1<-rep(c("astrocytes"),each=173) #astrocytes
#   x2<-rep(c("neoplastic"),each=174) #glioblastoma 1 patients lab data + noise
# 
#   l1 <- str_split(se_combined.subset@assays$SCT@data@Dimnames[[2]], '"')
#   dd1 <- c(l1[1:173])
#   dd2 <- c(l1[j])
#   v1 <- c(1:173) #random names for the noise
#   dd2 <- c(dd2, v1)
# 
