...
dataset2 <- data_newexp@meta.data[data_newexp@meta.data$cell_type == "monocyte",] #monocytes cell types
...
#combine datasets
se_combined.subset <- merge(se_m.subset, y = se_neo.subset, add.cell.ids = c("monocytes", "neoplastic"))
...
mmatrix <- GetAssayData(object = se_combined.subset, assay= "SCT", slot = "data") #get the normalized UMI counts for both GBM and monocytes


library(SCOPfunctions)
data <- utils_big_as.matrix(mmatrix)
nc <- ncol(data[,1:14215]) + 14216
new_mat <- matrix(0, ncol=nc,nrow=nrow(data[,1:14215]))
new_mat[1:nrow(data), 1:ncol(data[,1:14215])] <- data[,1:14215] #new matrix with the values from old one (with monocytes) and the rest 0 to add noise

number_fake_cells = 14215
gamma = 0.9
r1 <- 14217
c1 <- 28431
...
# setwd('C:/Users/PAULA CAMARGO ROMERA/OneDrive - Uppsala universitet/Master/Uppsala University/Courses/Year 2/Thesis/processing/cellphonedb_results/gbmap_celltypes/monocytes')
# p <- 0
# for (j in 14216:14265){
#   p <- p + 1
#   x1<-rep(c("monocytes"),each=14215) #monocytes
#   x2<-rep(c("neoplastic"),each=14216) #glioblastoma 1 patients lab data + noise
# 
#   l1 <- str_split(se_combined.subset@assays$SCT@data@Dimnames[[2]], '"')
#   dd1 <- c(l1[1:14215])
#   dd2 <- c(l1[j])
#   v1 <- c(1:14215) #random names for the noise
#   dd2 <- c(dd2, v1)
# 
