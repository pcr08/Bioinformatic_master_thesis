...
dataset2 <- data_newexp@meta.data[data_newexp@meta.data$cell_type == "natural killer cell",] # natural killer cell types
...
#GBmap data -- natural killer cells
# plot histogram unique genes
...
#combine datasets
se_combined.subset <- merge(se_m.subset, y = se_neo.subset, add.cell.ids = c("killer_cells", "neoplastic"))
...
mmatrix <- GetAssayData(object = se_combined.subset, assay= "SCT", slot = "data") #get the normalized UMI counts for both GBM and natural killer cells


library(SCOPfunctions)
data <- utils_big_as.matrix(mmatrix)

nc <- ncol(data[,1:2489]) + 2490
new_mat <- matrix(0, ncol=nc,nrow=nrow(data[,1:2489]))
new_mat[1:nrow(data), 1:ncol(data[,1:2489])] <- data[,1:2489] #new matrix with the values from old one (with natural killer cells) and the rest 0 to add noise

number_fake_cells = 2489
gamma = 0.9
r1 <- 2491
c1 <- 4979
...
# setwd('C:/Users/PAULA CAMARGO ROMERA/OneDrive - Uppsala universitet/Master/Uppsala University/Courses/Year 2/Thesis/processing/cellphonedb_results/gbmap_celltypes/killer_cells')
# p <- 0
# for (j in 2490:2539){
#   p <- p + 1
#   x1<-rep(c("killer_cells"),each=2489) #natural killer cells
#   x2<-rep(c("neoplastic"),each=2490) #glioblastoma 1 patients lab data + noise
# 
#   l1 <- str_split(se_combined.subset@assays$SCT@data@Dimnames[[2]], '"')
#   dd1 <- c(l1[1:2489])
#   dd2 <- c(l1[j])
#   v1 <- c(1:2489) #random names for the noise
#   dd2 <- c(dd2, v1)
# 
