...
dataset2 <- data_newexp@meta.data[data_newexp@meta.data$cell_type == "mast cell",] #mast cell types
...
#combine datasets
se_combined.subset <- merge(se_m.subset, y = se_neo.subset, add.cell.ids = c("mast_cells", "neoplastic"))
...
mmatrix <- GetAssayData(object = se_combined.subset, assay= "SCT", slot = "data") #get the normalized UMI counts for both GBM and mast cells


library(SCOPfunctions)
data <- utils_big_as.matrix(mmatrix)

nc <- ncol(data[,1:373]) + 374
new_mat <- matrix(0, ncol=nc,nrow=nrow(data[,1:373]))
new_mat[1:nrow(data), 1:ncol(data[,1:373])] <- data[,1:373] #new matrix with the values from old one (with mast cells) and the rest 0 to add noise

number_fake_cells = 373
gamma = 0.9
r1 <- 375
c1 <- 747
...
# setwd('C:/Users/PAULA CAMARGO ROMERA/OneDrive - Uppsala universitet/Master/Uppsala University/Courses/Year 2/Thesis/processing/cellphonedb_results/gbmap_celltypes/mast_cells')
# p <- 0
# for (j in 374:423){
#   p <- p + 1
#   x1<-rep(c("mast_cell"),each=373) #mast cells
#   x2<-rep(c("neoplastic"),each=374) #glioblastoma 1 patients lab data + noise
# 
#   l1 <- str_split(se_combined.subset@assays$SCT@data@Dimnames[[2]], '"')
#   dd1 <- c(l1[1:373])
#   dd2 <- c(l1[j])
#   v1 <- c(1:373) #random names for the noise
#   dd2 <- c(dd2, v1)
# 
