...
dataset2 <- data_newexp@meta.data[data_newexp@meta.data$cell_type == "neuron",] #neurons cell types
...
#combine datasets
se_combined.subset <- merge(se_m.subset, y = se_neo.subset, add.cell.ids = c("neurons", "neoplastic"))
...
mmatrix <- GetAssayData(object = se_combined.subset, assay= "SCT", slot = "data") #get the normalized UMI counts for both GBM and neurons


library(SCOPfunctions)
data <- utils_big_as.matrix(mmatrix)

nc <- ncol(data[,1:22]) + 23
new_mat <- matrix(0, ncol=nc,nrow=nrow(data[,1:22]))
new_mat[1:nrow(data), 1:ncol(data[,1:22])] <- data[,1:22] #new matrix with the values from old one (with neurons) and the rest 0 to add noise

number_fake_cells = 22
gamma = 0.9
r1 <- 24
c1 <- 45
...
# setwd('C:/Users/PAULA CAMARGO ROMERA/OneDrive - Uppsala universitet/Master/Uppsala University/Courses/Year 2/Thesis/processing/cellphonedb_results/gbmap_celltypes/neurons')
# p <- 0
# for (j in 23:72){
#   p <- p + 1
#   x1<-rep(c("neurons"),each=22) #neurons
#   x2<-rep(c("neoplastic"),each=23) #glioblastoma 1 patients lab data + noise
# 
#   l1 <- str_split(se_combined.subset@assays$SCT@data@Dimnames[[2]], '"')
#   dd1 <- c(l1[1:22])
#   dd2 <- c(l1[j])
#   v1 <- c(1:22) #random names for the noise
#   dd2 <- c(dd2, v1)
# 
